...
extern void gc(struct VMState *vm)
{
  ...
  growheap(target, count.current.pages);
...
{
  bool grew = false;
  while ((count.available.pages + count.current.pages) <= nlive * gamma)
  {
  ...
  availability_floor = (count.current.pages + count.available.pages) / 2 + 1;
}
