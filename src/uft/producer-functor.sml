...
fun produce p inputs =
  ...
  infix 1 <|>
  fun t1 <|> t2 = (fn xs => case t1 xs of SOME y => SOME y 
                                          | NONE => t2 xs) 
  ...
  fun sepBy p sep = curry (op ::) <$> p <~> sep <*> (fn xs => (sepBy p sep) xs) <|> succeed []

  fun sepBy1 p sep = curry (op ::) <$> p <~> sep <*> sepBy p sep

  fun comma (#","::xs) = SOME (Error.OK #",", xs)
    | comma _         = NONE
   

  (* fun commaSep p = sepBy1 p comma *)
