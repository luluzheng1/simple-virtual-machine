...
struct
  ...
  val binary  = [ "+", "-", "*", "/", "<", ">", "cons", "=", "eq", "<=", "idiv", "&&", "||"]
  val unary   = [ "boolean?", "null?", "number?", "pair?", "function?", "nil?"
                , "symbol?", "car", "cdr", "~", "!", "Bool", "zero"
                ]
  ...
  val side_effecting = [ "print", "printu", "println", "set-car!", "set-cdr!"]
  val no_reg_effects = [ "gc" ]
  val error          = [ "error" ]
  ...
  val primitives : primitive list =  (* you can also extend this definition *)
    ...
    o add 0 HAS_EFFECT no_reg_effects
    ) []
