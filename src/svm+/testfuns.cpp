...
static struct VMFunction *newfunction(int instruction_count)
{
  ...
  struct VMFunction *fun = (struct VMFunction *)malloc(vmsize_fun(instruction_count + 1));
  fun->arity = 0;
  fun->size = instruction_count + 1;
  Opcode h = Halt;
  fun->instructions[instruction_count] = eR0(h);
  return fun;
}
...
struct VMFunction *print0fun(void)
{
  ...
  fun->instructions[0] = eR1(opcode::Print, 0);
  return fun;
}

struct VMFunction *ce0fun(VMStateP vm)
{
  VMStringP r0string = VMString_newc("$r0");
  int stringslot = literal_slot(vm, mkStringValue(r0string));
  struct VMFunction *fun = newfunction(2);
  fun->instructions[0] = eR1U16(opcode::Check, 0, stringslot);
  fun->instructions[1] = eR1U16(opcode::Expect, 0, stringslot);
  return fun;
}
