...
bool identical(Value v1, Value v2)
{ // object identity,  for hashing!
  if (v1.tag != v2.tag)
    return false;
  else
    switch (v1.tag)
    ...
    case Emptylist:
      return true;
    case String:
      return v1.s == v2.s; // all strings assumed interned
    case Table:
      return v1.table == v2.table; // object identity
    case Seq:
      return v1.seq == v2.seq; // object identity from here on out
    case ConsCell:
...
uint32_t hashvalue(Value v)
{
  ...
  case Nil:
    return NILHASH;
  case Boolean:
    return v.b ? TRUEHASH : FALSEHASH;
  case Number:
    return NUMHASH ^ (uint32_t)v.n;
  case Emptylist:
    return EMPTYHASH;
  case String:
    return VMString_hash(v.s);
  case Table:
    return TABHASH;
  case Seq:
    return SEQHASH;
  ...
  case VMClosure:
    return CLOHASH;
  case LightUserdata:
    return (uint32_t)((uintptr_t)v.p >> 3);
  default:
    assert(0);
  }
}
...
bool eqvalue(Value v1, Value v2)
{ // Will not work for hashing!
  if (v1.tag != v2.tag)
    return false;
  else
    switch (v1.tag)
    ...
    case Table:
      return v1.table == v2.table; // object identity
    case Seq:
      return v1.seq == v2.seq; // object identity
    case ConsCell:
      return false;
    case Emptylist:
      return true;
...
bool eqtests(Value v1, Value v2)
{ // will not work for hashing!
