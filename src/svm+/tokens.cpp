...
struct tokens
{ // cons cell for linked list of tokens
  enum tokentype type;
  union
  {
    Name name;
...
{
  Tokens t = (Tokens)malloc(sizeof(*t));
  assert(t);
  t->type = TU32;
  t->n = n;
  return t;
...
void print_tokens(FILE *fp, Tokens toks)
{
  if (toks)
  {
    ...
    {
    case TNAME:
      fprintf(fp, "name:%s", nametostr(toks->name));
      break;
    case TU32:
      fprintf(fp, "int:%" PRIu32, toks->n);
      break;
    case TDOUBLE:
      fprintf(fp, "double:%g", toks->x);
      break;
    }
    fprintf(fp, " -> ");
...
static void invalid(const char *asked, const char *got, const char *original)
{
  assert(asked);
  assert(got);
...
double tokens_get_signed_number(Tokens *p, const char *original)
{
  ...
  case TU32:
    *p = ts->next;
    return (double)(int32_t)ts->n;
  case TDOUBLE:
    *p = ts->next;
    return ts->x;
  case TNAME:
    invalid("a number", "a name", original);
  default:
    assert(0);
    return 0.0;
  }
