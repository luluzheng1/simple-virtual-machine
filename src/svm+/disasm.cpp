...
void printasm(FILE *fp, VMStateP vm, Instruction i)
{
...
void idump(FILE *fp, VMStateP vm, int pc, Instruction I,
           long int regbase,
           Value *RX, Value *RY, Value *RZ)
{
  ...
  {
    assert(RX);
    fprint(fp, "  r%d = %V", uX(I), *RX);
  }
  if (used & has_uY)
  {
    assert(RY);
    fprint(fp, "  r%d = %V", uY(I), *RY);
  }
  if (used & has_uZ)
  {
    assert(RZ);
    fprint(fp, "  r%d = %V", uZ(I), *RZ);
  }
